 ...
 */
package com.example.bowon.graduationworkdebug.gui;
...
// 스크린 출력만을 담당하는 클래스
// 안드로이드  화면 출력을 위한 수치값 계산
// 켄버스 클레스를 따로 떄서 어느정도 커스터마이징한 것 일반 Canvas클레스와 큰 차이는 없다.
// 캔버스에 데이터를 그리는대 있어 전체적으로 사용되며 ScreenObj 클레스를 통해 추상화되어 사용된다.
public class PaintScreen {

	Canvas canvas;	// 출력에 사용될 캔버스 객체
	int width, height;	// 안드로이드 화면의 넓이와 높이
